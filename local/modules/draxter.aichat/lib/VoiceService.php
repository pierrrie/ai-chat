...
class VoiceService
{
    ...
    public static function transcribe(string $tmpPath, string $mimeType): array
    {
        ...
        $provider = self::resolveSttProvider();
        if ($provider === 'browser') {
            throw new \RuntimeException('Распознавание выполняется в браузере — загрузка аудио на сервер не требуется');
        }

        if ($provider === 'gemini') {
            $text = self::transcribeGemini($tmpPath, $mimeType);
        } elseif (str_starts_with($provider, 'custom:')) {
            $customId = substr($provider, 7);
            $cfg = CustomVoiceProviders::getSttById($customId);
            if ($cfg === null) {
                throw new \RuntimeException('Провайдер распознавания речи не найден: ' . $customId);
            }
            switch ($cfg['kind']) {
                case 'gemini':
                    $text = self::transcribeGeminiWithProvider($tmpPath, $mimeType, $cfg);
                    break;
                case 'http':
                    $text = self::transcribeHttp($tmpPath, $mimeType, $cfg);
                    break;
                default:
                    throw new \RuntimeException('Неизвестный тип STT: ' . $cfg['kind']);
            }
        } else {
            throw new \RuntimeException('STT не настроен');
        }
    ...
    /**
     * @return array{enabled: bool, reply: bool, stt: string, sttPref?: string, tts: bool, ttsProvider: string, sttError?: string}
     */
    public static function status(): array
    {
        ...
        $status = [
            ...
            'stt' => $stt,
            'sttPref' => Settings::voiceSttProvider(),
    ...
    {
        return Settings::resolveVoiceSttProvider();
    }
     ...
     */
    private static function transcribeGeminiWithProvider(string $tmpPath, string $mimeType, array $provider): string
    {
        ...
        if ($key === '') {
            throw new \RuntimeException('Укажите API-ключ Gemini для этого STT-провайдера');
        }
        $model = trim((string)($provider['model'] ?? ''));
        if ($model === '') {
            return self::transcribeGemini($tmpPath, $mimeType);
        }
    ...
    /**
     * Универсальный STT: POST multipart audio на URL провайдера.
     *
     * @param array<string, mixed> $provider
     */
    private static function transcribeHttp(string $tmpPath, string $mimeType, array $provider): string
    {
        $url = trim((string)($provider['url'] ?? ''));
        if ($url === '') {
            throw new \RuntimeException('Укажите URL API для STT-провайдера');
        }
        ...
        $mime = self::normalizeAudioMime($mimeType);
        $ext = self::audioExtensionForMime($mime);
        $fileField = class_exists(\CURLFile::class)
            ? new \CURLFile($tmpPath, $mime, 'audio.' . $ext)
            : '@' . $tmpPath;
        ...
        $lang = trim((string)($provider['lang'] ?? ''));
        if ($lang !== '') {
            $post['language'] = $lang;
        }

        $headers = self::buildAuthHeaders($auth, $key, $extraHeaders);
        ...
        if ($code === 200 && is_string($body) && $body !== '') {
            $text = self::extractTextFromHttpResponse($body);
            if ($text !== '') {
                return $text;
            }
        }
        ...
        throw new \RuntimeException('STT HTTP error ' . $code . $detail);
    }

    private static function extractTextFromHttpResponse(string $body): string
    {
        $json = json_decode($body, true);
        if (is_array($json)) {
            foreach (['text', 'result', 'transcript', 'transcription'] as $key) {
                if (!empty($json[$key]) && is_string($json[$key])) {
                    return trim($json[$key]);
                }
            }
            if (!empty($json['data']['text']) && is_string($json['data']['text'])) {
                return trim($json['data']['text']);
            }
            if (!empty($json['choices'][0]['text']) && is_string($json['choices'][0]['text'])) {
        ...
        return $plain !== '' && !str_starts_with($plain, '{') ? $plain : '';
    }
     ...
     * @return string[]
     */
    private static function buildAuthHeaders(string $auth, string $key, array $extraHeaders = []): array
    {
        $headers = [];
        if ($key !== '') {
            if ($auth === 'api-key') {
